...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return '该发生器生成一个控制器和视图实现CRUD(创建、读取、更新、删除) 　　操作指定的数据模型。已由Dreamzml改造';
    }
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field(\$model, '$attribute')->passwordInput()->label(\$model->getAttributeLabel('$attribute').' ：')";
            } else {
                return "\$form->field(\$model, '$attribute')->label(\$model->getAttributeLabel('$attribute').' ：')";
            }
        }
        $column = $tableSchema->columns[$attribute];
        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute')->checkbox()->label(\$model->getAttributeLabel('$attribute').' ：')";
        } elseif ($column->type === 'text') {
            return "\$form->field(\$model, '$attribute')->textarea(['rows' => 6])->label(\$model->getAttributeLabel('$attribute').' ：')";
        } else {
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field(\$model, '$attribute')->$input()->label(\$model->getAttributeLabel('$attribute').' ：')";
            } else {
                return "\$form->field(\$model, '$attribute')->$input(['maxlength' => $column->size])->label(\$model->getAttributeLabel('$attribute').' ：')";
            }
        }
    }
    ...
    public function generateActiveSearchField($attribute)
    {
        ...
        if ($column->phpType === 'boolean') {
            ...
            return "\$form->field(\$model, '$attribute')->textInput(['placeholder' => \$model->getAttributeLabel('$attribute')])";
        }
    }
