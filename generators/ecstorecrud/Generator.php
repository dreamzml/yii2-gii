...
class Generator extends \yii\gii\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'tableName', 'modelClass', 'moduleName', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass', 'controllerClass'], 'filter', 'filter' => 'trim'],
            [['ns', 'queryNs'], 'filter', 'filter' => function($value) { return trim($value, '\\'); }],

            [['db', 'ns', 'tableName', 'baseClass', 'queryNs', 'queryBaseClass', 'controllerClass'], 'required'],
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            //控制器
            $viewPath = $this->getControllerPath();
            $modelPath = "{$viewPath}.php";
            $files[] = new CodeFile($modelPath, $this->render('controller.php', $params));
            ...
            //模型,model
            $viewPath = $this->getFinderPath();
            $modelPath = "{$viewPath}.php";
            $files[] = new CodeFile($modelPath, $this->render('finder.php', $params));

            //接口,api
            $viewPath = $this->getApiPath();
            $modelPath = "{$viewPath}.php";
            $files[] = new CodeFile($modelPath, $this->render('api.php', $params));

            list($tables_name, $dbschema)  = $this->getDbschemaFile();
            $modelPath = "{$dbschema}.php";
            $params['tables_name'] = $tables_name;
            $files[] = new CodeFile($modelPath, $this->render('dbschema.php', $params));
        }
    ...
    {
        $dirStr = str_replace('_ctl_', '_controller_', $this->controllerClass);
        return $this->getBasePath().'/'.str_replace('_', '/', $dirStr);
    }
    ...
    {
        $dirStr = str_replace('_mdl_', '_model_', $this->modelClass);
        return $this->getBasePath().'/'.str_replace('_', '/', $dirStr);
    }
    ...
    {
        $cNameArr = explode('_', $this->controllerClass);
        $appid = array_shift($cNameArr);
        $ctl = array_shift($cNameArr);
        $dirs = join('/',$cNameArr);
        return $this->getBasePath().'/'.$appid.'/view/'.$dirs;
    }

    public function getBasePath(){
        return "E:/siteProgram/phpProject/yimi_svn/custom";
    }
     ...
     */
    public function getDbschemaFile(){
        $tableArr = explode('_', $this->tableName);
        $prefix = array_shift($tableArr);
        $appid = array_shift($tableArr);
        $tables = join('_',$tableArr);

        return array($tables, $this->getBasePath().'/'.$appid.'/dbschema/'.$tables);
    }
     ...
     * @return [type] [description]
     */
    public function getFinderPath(){
        $cNameArr = explode('_', $this->controllerClass);
        $appid = array_shift($cNameArr);
        $ctl = array_shift($cNameArr);
        $dirs = join('/',$cNameArr);

        return $this->getBasePath().'/'.$appid.'/lib/finder/'.$dirs;
    }
    ...
    public function getApiPath(){
        $cNameArr = explode('_', $this->tableName);
        $prefix = array_shift($cNameArr);
        $appid = array_shift($cNameArr);
        $dirs = join('/',$cNameArr);

        return $this->getBasePath().'/'.$appid.'/lib/apiv/apis/response/'.$dirs;
    }
}
