...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return 'discuzplugin plugin generotor.';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'pluginName', 'modelClass', 'moduleName', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass', 'controllerClass'], 'filter', 'filter' => 'trim'],
            ...
            [['pluginName'], 'match', 'pattern' => '/^(\w+\.)?([\w\*]+)$/', 'message' => 'Only word characters, and optionally an asterisk and/or a dot are allowed.'],
            [['db'], 'validateDb'],
            [['ns', 'queryNs'], 'validateNamespace'],
            [['moduleName', 'modelClass'], 'safe'],
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            'pluginName' => 'This is the name of the DB table that the new ActiveRecord class is associated with, e.g. <code>post</code>.
            ...
            'moduleName' => '模块名称.User,System,Wap,Home',
    ...
    public function generate()
    {
        ...
        //         $files[] = new CodeFile("{$viewPath}/{$fileName}", $this->render("views/$file", $params));
        //     }
        // }
        ...
        $pluginName = 'myapp2';
        $basePath = "{$basePath}/{$pluginName}/";

        //自述文件
        $modelPath = "{$basePath}discuz_plugin_{$pluginName}.xml";
        $files[] = new CodeFile($modelPath, $this->render('readme.php', $params));
    ...
    private function generateManyManyRelations($table, $fks, $relations)
    {
        ...
        $relations[$table0Schema->fullName][$relationName] = [
            ...
            . $this->generatepluginName($table->name) . "', $viaLink);",
            $className1,
            true,
        ];
    ...
    protected function generateRelations()
    {
        ...
        if (!isset($schemaNames)) {
            if (($pos = strpos($this->pluginName, '.')) !== false) {
                $schemaNames = [substr($this->pluginName, 0, $pos)];
            } else {
                $schemaNames = [''];
            }
        }
    ...
    public function validateModelClass()
    {
        ...
        if ((empty($this->pluginName) || substr_compare($this->pluginName, '*', -1, 1)) && $this->modelClass == '') {
     ...
     * @param string $pluginName the table name (which may contain schema prefix)
     * @return string the generated table name
     */
    public function generatepluginName($pluginName)
    {
        ...
        $db = $this->getDbConnection();
        if (preg_match("/^{$db->tablePrefix}(.*?)$/", $pluginName, $matches)) {
            $pluginName = '{{%' . $matches[1] . '}}';
        } elseif (preg_match("/^(.*?){$db->tablePrefix}$/", $pluginName, $matches)) {
            $pluginName = '{{' . $matches[1] . '%}}';
        }
        return $pluginName;
    }
     ...
     */
    protected function generateClassName($pluginName, $useSchemaName = null)
    {

        if (isset($this->classNames[$pluginName])) {
            return $this->classNames[$pluginName];
        }

        $schemaName = '';
        $fullpluginName = $pluginName;
        if (($pos = strrpos($pluginName, '.')) !== false) {
            if (($useSchemaName === null && $this->useSchemaName) || $useSchemaName) {
                $schemaName = substr($pluginName, 0, $pos) . '_';
            }
        ...
        $patterns[] = "/^(.*?){$db->tablePrefix}$/";
        if (strpos($this->pluginName, '*') !== false) {
            $pattern = $this->pluginName;
        ...
        foreach ($patterns as $pattern) {
            if (preg_match($pattern, $pluginName, $matches)) {
                $className = $matches[1];
                break;
            }
        }

        return $this->classNames[$fullpluginName] = Inflector::id2camel($schemaName.$className, '_');
    }
    ...
    protected function generateQueryClassName($modelClassName)
    {
        ...
        if (empty($queryClassName) || strpos($this->pluginName, '*') !== false) {
    ...
    public function getBasePath(){
        return "E:/siteProgram/phpProject/discuz/source/plugin";
    }
